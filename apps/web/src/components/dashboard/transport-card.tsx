    ...
    <Tooltip delayDuration={250}>
      <TooltipTrigger asChild>
        <button
          ...
          className={cn(
            "group bg-ink-100 hover:bg-ink-150 relative flex w-full items-center gap-2 rounded-md border-l-4 py-2 pr-2.5 pl-3 text-left ring-1 ring-white/[0.04] transition-colors duration-150 hover:ring-white/10",
            meta.borderClass,
      ...
      <TooltipContent side="top" className="font-mono">
        {firstNameAndInitial(transport.patientName)} · {meta.label}
        {deadlineRelative && (
          <>
            <br />
            <span className="text-zinc-400">deadline {deadlineRelative}</span>
          </>
        )}
        <br />
        <span className="text-zinc-400">{TRIP_LABEL[transport.tripType]}</span>
      </TooltipContent>
    </Tooltip>
  );
}
